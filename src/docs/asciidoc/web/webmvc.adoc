...
----
	...
	class PersonController {
		...
		public void add(@RequestBody Person bean) {
			// ...
		}
	}
----
...
----
	...
	class PersonController {
		...
		@PostMapping
		@ResponseStatus(HttpStatus.CREATED)
		fun add(@RequestBody bean: Person) {
			// ...
		}
	}
----
...
controller mapped to `/bean` is also implicitly mapped to `/bean.{asterisk}`.
The file extension is then used to interpret the requested content type to use for
the response (that is, instead of the `Accept` header) -- for example, `/bean.pdf`,
`/bean.xml`, and others.
