...
.Java
----
	Person bean = client.get().uri("/bean/{id}", i).retrieve()
		.bodyToMono(Person.class)
		.block();
...
.Kotlin
----
	val bean = runBlocking {
		client.get().uri("/bean/{id}", i).retrieve()
				.awaitBody<Person>()
	}
...
.Java
----
	Mono<Person> personMono = client.get().uri("/bean/{id}", personId)
			.retrieve().bodyToMono(Person.class);

	Mono<List<Hobby>> hobbiesMono = client.get().uri("/bean/{id}/hobbies", personId)
			.retrieve().bodyToFlux(Hobby.class).collectList();

	Map<String, Object> data = Mono.zip(personMono, hobbiesMono, (bean, hobbies) -> {
...
----
	val data = runBlocking {
			val personDeferred = async {
				client.get().uri("/bean/{id}", personId)
						.retrieve().awaitBody<Person>()
			}

			val hobbiesDeferred = async {
				client.get().uri("/bean/{id}/hobbies", personId)
						.retrieve().bodyToFlow<Hobby>().toList()
			}

			mapOf("bean" to personDeferred.await(), "hobbies" to hobbiesDeferred.await())
		}
----
