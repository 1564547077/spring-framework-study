...
----
	...
	RouterFunction<ServerResponse> route = route()
		.GET("/bean/{id}", accept(APPLICATION_JSON), handler::getPerson)
		.GET("/bean", accept(APPLICATION_JSON), handler::listPeople)
		.POST("/bean", handler::createPerson)
		.build();
...
----
	...
	val route = coRouter { // <1>
		accept(APPLICATION_JSON).nest {
			GET("/bean/{id}", handler::getPerson)
			GET("/bean", handler::listPeople)
		}
		POST("/bean", handler::createPerson)
	}
...
----
Mono<Person> bean = ...
ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).body(bean, Person.class);
----
[source,kotlin,role="secondary"]
.Kotlin
----
val bean: Person = ...
ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).bodyValue(bean)
----
...
public class PersonHandler {
	...
	public Mono<ServerResponse> createPerson(ServerRequest request) { // <2>
		Mono<Person> bean = request.bodyToMono(Person.class);
		return ok().build(repository.savePerson(bean));
	}

	public Mono<ServerResponse> getPerson(ServerRequest request) { // <3>
		int personId = Integer.valueOf(request.pathVariable("id"));
		return repository.getPerson(personId)
			.flatMap(bean -> ok().contentType(APPLICATION_JSON).bodyValue(bean))
...
a completion signal when the bean has been read from the request and stored. So we use the
...
<3> `getPerson` is a handler function that returns a single bean, identified by the `id` path
...
----
	class PersonHandler(private val repository: PersonRepository) {
		...
		suspend fun createPerson(request: ServerRequest): ServerResponse { // <2>
			val bean = request.awaitBody<Person>()
			repository.savePerson(bean)
...
----
	public class PersonHandler {
		...
		public Mono<ServerResponse> createPerson(ServerRequest request) {
			Mono<Person> bean = request.bodyToMono(Person.class).doOnNext(this::validate); // <2>
			return ok().build(repository.savePerson(bean));
		}
...
----
	class PersonHandler(private val repository: PersonRepository) {
		...
		private fun validate(bean: Person) {
			val errors: Errors = BeanPropertyBindingResult(bean, "bean");
			validator.validate(bean, errors);
			if (errors.hasErrors()) {
...
RouterFunction<ServerResponse> route = route()
	.GET("/bean/{id}", accept(APPLICATION_JSON), handler::getPerson) // <1>
	.GET("/bean", accept(APPLICATION_JSON), handler::listPeople) // <2>
	.POST("/bean", handler::createPerson) // <3>
	.add(otherRoute) // <4>
	.build();
----
<1> pass:q[`GET /bean/{id}`] with an `Accept` header that matches JSON is routed to
`PersonHandler.getPerson`
<2> `GET /bean` with an `Accept` header that matches JSON is routed to
`PersonHandler.listPeople`
<3> `POST /bean` with no additional predicates is mapped to
...
matches `/bean`, used by three of the routes. When using annotations, you would remove
this duplication by using a type-level `@RequestMapping` annotation that maps to
`/bean`. In WebFlux.fn, path predicates can be shared through the `path` method on the
...
----
	...
	val route = router {
			("/bean" and accept(APPLICATION_JSON)).nest {
				GET("/{id}", handler::getPerson)
				GET("", handler::listPeople)
				POST("/bean", handler::createPerson)
				filter { request, next ->
